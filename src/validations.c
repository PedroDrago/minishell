...
{
	ft_putstr_fd("Minishell: syntax error near unexpected token `", 2), ft_putchar_fd(c, 2), ft_putstr_fd("'\n", 2);
	return (FALSE);
}
...
{
	ft_putstr_fd("Minishell: syntax error: Invalid character:`", 2), ft_putchar_fd(c, 2), ft_putstr_fd("'\n", 2);
	return (FALSE);
}
...
int	validate_prompt(char *prompt, t_shell *shell)
{
	if (!prompt || !ft_strlen(prompt))
		return (FALSE);
	if (!validate_quotes(prompt) || !validate_double_tokens(prompt) || !validate_edge_tokens(prompt) || !validate_characters(prompt))
	{
